...
StyleDictionary.registerFormat({
    ...
    formatter: _.template(
        fs.readFileSync(__dirname + '/varsvarsvars.template')
    ),
});
...
StyleDictionary.registerFormat({
    ...
    formatter: function ({ dictionary, options }) {
        ...
        dictionary.allProperties.map((prop) => {
            if (!dict[prop.attributes.category])
                dict[prop.attributes.category] = {};
                ...
                if (dictionary.usesReference(v.original.value)) {
                    ...
                    reference.forEach((ref) => {
                        ...
                        if (
                            ref.value &&
                            ref.name &&
                            ref.value.replace === 'Function'
                        ) {
                            v.value = ref.value.replace(ref.value, function () {
                                return `var(--${ref.name})`;
                            });
                        }

                        return `var(--global-notdefined)`;
                    });
